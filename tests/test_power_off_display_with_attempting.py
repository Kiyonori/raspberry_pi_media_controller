...
def test_power_off_display_with_attempting_はディスプレイの電源状態が_POWERED_ON_である場合_内部からtoggle_display_power_statusメソッドを呼んで終了を試みる最大回数は10回であること():
    # get_wattages_on_display をモック化して
    # 数秒間の測定なしに、即時に固定のワット数を返すようにして、テストを高速化
    with patch(
            'raspberry_pi_media_controller.power_off_display_with_attempting.get_wattages_on_display'
        ...
        with patch(
            ...
            with patch(
                ...
                # ℹ️
                mock_get_display_power_status.return_value = DisplayPowerStatusEnum.POWERED_ON
