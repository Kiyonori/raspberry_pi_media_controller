...
def test_power_on_display_with_attempting_はディスプレイの電源状態が_POWERED_OFF_AND_SIGNAL_CAN_BE_RECEIVED_であり_内部からtoggle_display_power_statusメソッドを複数回呼び_状態がPOWERED_ONになるまで電源投入を試みること():
    # get_wattages_on_display をモック化して
    # 数秒間の測定なしに、即時に固定のワット数を返すようにして、テストを高速化
    with patch(
            'raspberry_pi_media_controller.power_on_display_with_attempting.get_wattages_on_display'
...
def test_power_on_display_with_attempting_はディスプレイの電源状態が_POWERED_OFF_AND_SIGNAL_CAN_BE_RECEIVED_である場合_内部からtoggle_display_power_statusメソッドを呼んで電源投入を試みる最大回数は10回であること():
    # get_wattages_on_display をモック化して
    # 数秒間の測定なしに、即時に固定のワット数を返すようにして、テストを高速化
    with patch(
            'raspberry_pi_media_controller.power_on_display_with_attempting.get_wattages_on_display'
        ...
        with patch(
            ...
            with patch(
                ...
                main()
                assert mock_toggle_display_power_status.call_count == 10
