...
def test_power_on_media_player_with_attempting_はメディアプレイヤーの消費電力が異常な値を示している_TROUBLE_の場合_内部からtoggle_media_player_power_statusメソッドが一度も呼ばれないこと():
    # get_wattages_on_media_player をモック化して
    # 数秒間の測定なしに、即時に固定のワット数を返すようにして、テストを高速化
    with patch(
            'raspberry_pi_media_controller.power_on_media_player_with_attempting.get_wattages_on_media_player'
...
def test_power_on_media_player_with_attempting_はメディアプレイヤーの電源状態が_POWERED_OFF_AND_SIGNAL_CAN_BE_RECEIVED_である場合_内部からtoggle_media_player_power_statusメソッドを呼んで電源投入を試みる最大回数は10回であること():
    # get_wattages_on_media_player をモック化して
    # 数秒間の測定なしに、即時に固定のワット数を返すようにして、テストを高速化
    with patch(
            'raspberry_pi_media_controller.power_on_media_player_with_attempting.get_wattages_on_media_player'
        ...
        with patch(
            ...
            with patch(
                ...
                # ℹ️
                mock_get_media_player_power_status.return_value = MediaPlayerPowerStatusEnum.POWERED_OFF_AND_SIGNAL_CAN_BE_RECEIVED
